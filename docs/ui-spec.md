...
> 版本：1.3；日期：2026-09-24；角色：UI/UX 设计师（R2）。
> 事实来源：`docs/prd-m1.md`（功能与流程）、`docs/copy-m1.md`（全部界面文案，本文直接引用其 Key）、`docs/interface-contract.md` **3.0.0 / r4（已生效）**（数据字段、检测降级语义、进度阶段、错误与恢复动作）、`docs/architecture-m1.md` 第 9 节（UI 状态交接表）。
...
> 修订记录：v1.3（2026-09-24）按 [M1.0 热修设计咨询](./acceptance/consult-2026-09-24-m1.0-hotfix.md) Q1/Q2 承接（来源：集成验收后产品主实测缺陷 1「时间轴缩略图从不渲染」与缺陷 2「播放无渲染面」）：§5.3.4 新增动画预览视口（时间轴左侧固定 dock、方形、逻辑画布/letterbox/空帧占位）、播放行为（FPS 计时推进、playhead 与芯片/擦洗双向同步、编辑立即暂停、播放期零 RPC + 预取 ±8 帧）、洋葱皮规范（前 0.30/后 0.20 纯 α）、缩略图与播放位图数据路径五条护栏 + 共享帧裁剪位图服务（§5.3.4）；§5.3.6 写明导出预览与本视口的职责分工；状态清单补 C-72～C-74、设计发现补 D-09；时间轴总高维持 152px。契约 3.0.0 零变更。
...
| 时间轴 | 高 152px（v1.3 定值，总高不变）：左侧动画预览视口 dock 128×128（上下各 12px 边距）+ 12px 列间距 + 右列（头部 40 + 待复核摘要 26 + 芯片行 ~86） | 内容见 5.3.4 |
...
#### 5.3.4 时间轴（总高 152px，v1.3）

```
┌ 动画预览视口 128×128 ┐┌ 头部 40px ───────────────────────────────────────────────┐
│ （棋盘格底，当前帧    ││ [⏮][▶/⏸][⏭] [12 FPS] [洋葱皮⏀] …弹簧… [筛选帧: 全部帧(n) …] [16 帧] │
│  逻辑画布 letterbox， ├┤ 待复核摘要条 26px（条件显示）──────────────────────────────┤
│  左下角 frame_index） │├ 芯片行（水平滚动，高 ~86）─────────────────────────────────┤
│  dock 于时间轴左侧    ││ [①thumb][②thumb][③thumb▸]…                                │
└───────────────────────┘└──────────────────────────────────────────────────────────┘
```

**动画预览视口（v1.3 新增，M1.0 热修缺陷 2 的渲染面）**：

- **位置**：时间轴头部左侧固定 dock——渲染面与播放控件（F-15 交互簇）同址，形成「擦洗↔播放↔步进」最短操作回路；不随画布 pan/zoom 变化、不遮挡工作区、不在侧栏。
- **标签**：视口 + 播放控件整组以 `preview.title`（动画预览 / Animation preview）为组标题与 aria-label；v1.3 起该标题随视口落在 dock 区域，右列头部不再重复。
- **尺寸**：128×128 CSS px 方形（咨询建议区间 96–128 的取值），上下各 12px 边距内切于时间轴 152px 总高；与右列间距 12px；DPR 感知（位图按播放档 320px 预取，覆盖 DPR ≤ 2.5）。
- **底面**：透明棋盘格（同画布 8px 棋盘令牌）。
- **内容规则**：当前帧的逻辑画布（`Frame.canvas`）——uniform 模式取统一画布尺寸等比缩放居中（letterbox）；per-frame 模式取最大帧画布 letterbox；`bbox` 内容按 `canvas.offset` 摆放；空帧 = 纯棋盘格占位一拍（不发位图请求，时长与顺序保留）。视口左下角叠加 caption（mono）显示 `editor.frame_index`。
- **职责分工（v1.3，防混淆）**：本视口只做**动作的动态预览**（播放/擦洗/洋葱皮）；**选中帧的规范化摆放检查**由侧栏组 3「导出预览」（5.3.6）承担，两者不互换（见 5.3.6 组 3）。

**播放行为规范（v1.3）**：

- **计时**：按 FPS（`preview.fps`，1–120 步进边界，默认 12）逐帧推进，每帧等时长（契约 M1 语义）；循环播放回到第一帧。
- **playhead 与双向同步**：播放指针（playhead）当前帧的芯片显示 playhead 样式（`--accent` 顶部 2px 条 + 序号高亮），与「选中态」**相互独立**（播放不改变用户选中/多选）；芯片行自动滚动跟随 playhead（用户手动滚动时暂停跟随 2s）。反向：点击芯片 = playhead 跳转该帧并选中；`preview.previous` / `preview.next` 与 `,` / `.` 步进同样移动 playhead。
- **擦洗**：暂停态按住拖动芯片行 = 逐帧擦洗（视口实时跟随）；播放中拖动 = 立即转擦洗（暂停），松手**不**自动恢复播放；播放中单击芯片 = 跳转该帧并继续播放。
- **编辑打断（定稿采纳咨询建议）**：播放中发生任何改变时间轴/帧内容的操作（几何编辑、增删、合并、拆分、排序、撤销/重做、参数重算、手动网格应用）→ **立即暂停**；playhead 钳制到新时间轴最近有效帧序，不自动恢复播放（用户重新播放继续）。
- **播放期零 RPC**：播放循环本身不发起任何 Worker RPC。帧位图经 preview 通道预取，窗口 = 当前 playhead ±8 帧（后台空闲队列，见下文护栏）；未就绪帧**保持上一已就绪帧画面**，绝不阻塞等待 Worker、绝不黑屏跳变。

**洋葱皮规范（v1.3）**：

- 开关：时间轴头部开关（`preview.onion_skin`）或 `O` 键；开/关态 tooltip 用 `preview.onion_on` / `preview.onion_off`。
- 渲染：视口中当前帧之下叠加前 1 帧（α 0.30）与后 1 帧（α 0.20），**纯 alpha 叠加不染色**；前/后不存在（首/尾帧）只画存在侧。
- 与步进/擦洗共存：暂停逐帧（`,` / `.` / 按钮 / 拖动）时洋葱皮同样生效，用于帧间对齐检查。
...
**缩略图与播放位图·数据路径护栏（v1.3，M1.0 热修缺陷 1 的实现约束，咨询 Q1 全文落入）**：

1. **数据路径**：芯片缩略图经 `preview` 命令裁剪 `sourceRect = frame.bbox`、`maxDimension = 112`（56 CSS px × DPR2）；客户端按 `Frame.canvas` 几何（统一画布 + `canvas.offset`）合成到 56×56 棋盘格芯片；`bbox = null` 空帧不请求、纯棋盘格占位。首屏可先用 `load.preview`（≤1024 全图）垫底低清版，可见芯片再升级（两级为可选优化，时间紧可先只做升级档）。
2. **懒渲染**：仅渲染进入/接近视口的芯片（IntersectionObserver 或虚拟化），预取窗口 ≈ ±可见数；500 帧（desktop 上限）不一次性请求。
3. **缓存**：芯片档（112²×4 ≈ 48KB/张）与播放档（320²×4 ≈ 400KB/张）分列 LRU，合计 ≤ 主线程缓存预算 32MiB（架构 §5.2）；芯片档上限 512 张 ≈ 24MB。
4. **调度**：缩略图/预取请求走后台空闲队列串行提交（Worker 单任务）；用户发起的 detect/normalize/preview 优先，用户任务到来时挂起队列；响应回来时 `editRevision` / `normalizationId` 不匹配即丢弃。
5. **失效**：normalize 成功（`normalizationId` 变化）清空缓存重填；排序/移动不重取像素（缓存键 = frameId + 几何）。
6. **实现形态**：R5 抽共享「帧裁剪位图服务」模块——芯片缩略图、动画预览视口、侧栏组 3 导出预览共用同一裁剪/缓存/队列管线，禁止两套实现。

**辅助文案**（右列头部下方一行 caption，条件显示）：`editor.reorder_hint`（首次进入且帧 ≥2 时显示 8s）；`preview.empty`（零帧时芯片行替换为居中空状态文案，播放控件与视口禁用/置灰，视口显示纯棋盘格）。
...
- 预览数据随草稿变更后台自动重算（轻量 normalize 任务）；重算中预览区显示细进度线，不阻塞编辑；角标以重算完成后的 flags 为准。位图获取复用「帧裁剪位图服务」（5.3.4 护栏 6）。
- **职责分工（v1.3，防混淆）**：本组只做**选中帧的规范化摆放检查**（静态、单帧、随草稿后台重算）；**动作的动态预览**（播放/擦洗/洋葱皮）一律在时间轴左侧动画预览视口（5.3.4），侧栏不承担播放，本组也不渲染播放。
...
| C-49 | 动画预览控件与视口 | 时间轴头部（播放/步进/FPS/洋葱皮）+ 左侧动画预览视口（5.3.4，v1.3） | `preview.play/pause/previous/next/fps/onion_skin/onion_on/onion_off` | 播放控制 |
...
| C-72 | 视口首帧位图加载中（v1.3） | 视口显示棋盘格 + 细进度线；播放/擦洗不等待——未就绪帧保持上一已就绪帧画面（5.3.4 零 RPC 规则） | aria 用 `status.busy`；可见文字占位不强制（如需文字见 D-09 建议词条，走 PM） | 等待/正常操作不受阻 |
| C-73 | 播放/暂停态切换（v1.3） | 头部按钮图标与 aria 在 `preview.play` / `preview.pause` 间切换；播放中 playhead 芯片样式 + 芯片行跟随滚动；编辑立即暂停（5.3.4） | `preview.play`、`preview.pause`、`shortcuts.play_pause` | Enter / 按钮 |
| C-74 | 空帧播放占位（v1.3） | 空帧一拍 = 视口纯棋盘格占位（不发位图请求）；时长保留、时间轴顺序不变（对齐 AC-F09 场景 C 空帧语义） | 无新增（角标语境复用 `review.badge.empty`） | — |
...
| D-09 | 待 PM 决定（v1.3） | 动画预览视口首帧位图加载中（C-72）当前规格为「棋盘格 + 细进度线，aria 用 `status.busy`」——无可见文字；若 PM 希望有可见加载文案需补 copy | 建议词条 `preview.viewport_loading`：zh「正在载入预览帧…」/ en「Loading preview frame…」；不录入则维持无文字占位，不阻塞实现 | 低（可选增强） |
